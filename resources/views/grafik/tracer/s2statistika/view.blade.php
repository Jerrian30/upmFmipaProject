<div class="row">
    <div class="col-12 s2tatistikaContainer" style="display: none;">
        <h3>1. Riwayat Studi Alumni</h3>
    </div>
    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body" >
                <h6 class="card-title">a. Apakah Saudara berorganisasi ketika mahasiswa</h6>
                {!! $s2statis1->container() !!}
            </div>
        </div>
    </div>

    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">b. Sumber biaya saat kuliah</h6>
                {!! $s2statis2->container() !!}
            </div>
        </div>
    </div>
    
    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">c. IPK Lulusan</h6>
                {!! $s2statis3->container() !!}
            </div>
        </div>
    </div>

    <!-- Grafik Kedua -->
    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">d. Setelah lulus Sarjana, apakah melanjutkan studi lagi?</h6>
                {!! $s2statis4->container() !!}
            </div>
        </div>
    </div>
<br><br><br>
<div class="col-12 s2tatistikaContainer" style="display: none;">
    <h3>2. Riwayat Pekerjaan Alumni</h3>
</div>
    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">a. Waktu tunggu untuk mendapatkan pekerjaan</h6>
                {!! $s2statis5->container() !!}
            </div>
        </div>
    </div>

    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">b. Bergerak di bidang apakah pekerjaan/wirausaha Anda saat ini?</h6>
                {!! $s2statis6->container() !!}
            </div>
        </div>
    </div>

    <!-- Grafik Kedua -->
    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">c. Bagaimana proses  mendapatkan pekerjaan ini?</h6>
                {!! $s2statis7->container() !!}
            </div>
        </div>
    </div>
    
    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">d. Berapa pendapatan rata-rata per bulan  pada pekerjaan terakhir/sekarang?</h6>
                {!! $s2statis8->container() !!}
            </div>
        </div>
    </div>

    <!-- Grafik Kedua -->
    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">e. Apakah pekerjaan terkait dengan bidang ilmu yang Saudara pelajari?
                    </h6>
                {!! $s2statis9->container() !!}
            </div>
        </div>
    </div>

    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">f. Menurut Saudara, bagaimana kebutuhan institusi tempat Saudara bekerja terhadap lulusan dari Prodi/Jurusan Saudara</h6>
                {!! $s2statis10->container() !!}
            </div>
        </div>
    </div>
    
    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">g. Secara umum, apa pertimbangan utama Saudara dalam memilih pekerjaan yang terakhir/sekarang?</h6>
                {!! $s2statis11->container() !!}
            </div>
        </div>
    </div>
<br><br><br>
<div class="col-12 s2tatistikaContainer" style="display: none;">
    <h3>3. Kemampuan Lulusan</h3>
</div>
    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">a. Saat baru lulus, sejauh mana Saudara mampu bersaing dengan lulusan perguruan tinggi lain?</h6>
                {!! $s2statis12->container() !!}
            </div>
        </div>
    </div>

    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">b. Saat baru lulus, menurut penilaian Saudara, sejauh mana Saudara menguasai kompetensi berikut? [Kemampuan Etika]</h6>
                {!! $s2statis13->container() !!}
            </div>
        </div>
    </div>

    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">c. Saat baru lulus, menurut penilaian Saudara, sejauh mana Saudara menguasai kompetensi berikut? [Keahlian bidang ilmu utama]</h6>
                {!! $s2statis14->container() !!}
            </div>
        </div>
    </div>

    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">d. Saat baru lulus, menurut penilaian Saudara, sejauh mana Saudara menguasai kompetensi berikut? [Kemampuan berbahasa asing]</h6>
                {!! $s2statis15->container() !!}
            </div>
        </div>
    </div>

    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">e. Saat baru lulus, menurut penilaian Saudara, sejauh mana Saudara menguasai kompetensi berikut? [Penggunaan teknologi informasi]</h6>
                {!! $s2statis16->container() !!}
            </div>
        </div>
    </div>
    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">f. Saat baru lulus, menurut penilaian Saudara, sejauh mana Saudara menguasai kompetensi berikut? [Kemampuan berkomunikasi]</h6>
                {!! $s2statis17->container() !!}
            </div>
        </div>
    </div>
    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">g. Saat baru lulus, menurut penilaian Saudara, sejauh mana Saudara menguasai kompetensi berikut? [Kerjasama]</h6>
                {!! $s2statis18->container() !!}
            </div>
        </div>
    </div>
    <div class="col-lg-5 mb-4 s2tatistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">h. Saat baru lulus, menurut penilaian Saudara, sejauh mana Saudara menguasai kompetensi berikut? [Pengembangan diri]</h6>
                {!! $s2statis19->container() !!}
            </div>
        </div>
    </div>
</div> 
