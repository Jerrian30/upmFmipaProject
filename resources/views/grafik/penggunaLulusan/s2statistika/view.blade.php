
<div class="row">
    <div class="col-lg-3 mb-4 statistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body" >
                <h6 class="card-title">1. Memiliki kejujuran, etika pergaulan, keramahan dan sikap  yang baik</h6>
                {!! $statis1->container() !!}
            </div>
        </div>
    </div>

    <div class="col-lg-3 mb-4 statistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">2. Memiliki kemampuan dan menguasai statisg keilmuannya <br></h6>
                {!! $statis2->container() !!}
            </div>
        </div>
    </div>
    
    <div class="col-lg-3 mb-4 statistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">3. Memiliki kemampuan berbahasa   asing (inggris/ jepang/ perancis/ cina)</h6>
                {!! $statis3->container() !!}
            </div>
        </div>
    </div>

    <!-- Grafik Kedua -->
    <div class="col-lg-3 mb-4 statistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">4. Memiliki kemampuan menggunakan Teknologi Informasi dan Komunikasi</h6>
                {!! $statis4->container() !!}
            </div>
        </div>
    </div>

    <div class="col-lg-3 mb-4 statistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">5. Memiliki kemampuan berkomunikasi</h6>
                {!! $statis5->container() !!}
            </div>
        </div>
    </div>

    <div class="col-lg-3 mb-4 statistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">6. Memiliki kemampuan kerjasama dalam tim</h6>
                {!! $statis6->container() !!}
            </div>
        </div>
    </div>

    <!-- Grafik Kedua -->
    <div class="col-lg-3 mb-4 statistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">7. Memiliki kedisiplinan yang baik dalam berbagai aspek</h6>
                {!! $statis7->container() !!}
            </div>
        </div>
    </div>
    
    <div class="col-lg-3 mb-4 statistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">8. Memiliki rasa percaya diri yang tinggi</h6>
                {!! $statis8->container() !!}
            </div>
        </div>
    </div>

    <!-- Grafik Kedua -->
    <div class="col-lg-3 mb-4 statistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">9. Memiliki rasa tanggung jawab <br><br> <p style="opacity: 0">hi</p></h6>
                {!! $statis9->container() !!}
            </div>
        </div>
    </div>

    <div class="col-lg-3 mb-4 statistikaContainer" style="display: none;">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">10. Kemampuan menyampaikan ide pada statisg pekerjaan yang menjadi tanggung jawabnya</h6>
                {!! $statis10->container() !!}
            </div>
        </div>
    </div>
</div>