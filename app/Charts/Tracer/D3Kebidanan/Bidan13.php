...
class Bidan13
{
    ...
    {
        $data = Tracer::select('pekerjaan_sesuai_harapan', \DB::raw('count(*) as total'))
            ->where('program_studi', 'D3 Kebidanan') // Menambahkan filter untuk program_studi
            ->groupBy('pekerjaan_sesuai_harapan')
            ->pluck('total', 'pekerjaan_sesuai_harapan')->all();

        $labels = ['Tidak sesuai harapan', 'Kurang sesuai harapan', 'Sesuai harapan', 'Sangat sesuai dengan Harapan'];
