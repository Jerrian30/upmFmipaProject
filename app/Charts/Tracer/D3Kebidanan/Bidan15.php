...
class Bidan15
{
    ...
    {
        $data = Tracer::select('pendidikan_relevan_dengan_pekerjaan', \DB::raw('count(*) as total'))
            ->where('program_studi', 'D3 Kebidanan') // Menambahkan filter untuk program_studi
            ->groupBy('pendidikan_relevan_dengan_pekerjaan')
            ->pluck('total', 'pendidikan_relevan_dengan_pekerjaan')->all();
