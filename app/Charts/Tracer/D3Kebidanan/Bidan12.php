...
class Bidan12
{
    ...
    {
        $data = Tracer::select('tempat_bekerja_tenaga_kesehatan', \DB::raw('count(*) as total'))
            ->where('program_studi', 'D3 Kebidanan') // Menambahkan filter untuk program_studi
            ->groupBy('tempat_bekerja_tenaga_kesehatan')
            ->pluck('total', 'tempat_bekerja_tenaga_kesehatan')->all();

        $labels = ['Praktik Mandiri BIdan', 'Rumah Sakit Ibu dan Anak', 'Apotek', 'Puskesmas', 'Klinik Kesehatan', 'Rumah Sakit'];
