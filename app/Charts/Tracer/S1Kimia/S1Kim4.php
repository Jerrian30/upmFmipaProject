...
namespace App\Charts\Tracer\S1Kimia;
...
class S1Kim4
{
    ...
    {
        $data = Tracerr::select('melanjutkan_studi', \DB::raw('count(*) as total'))
            ->where('lulus_dari_program_studi', 'S1 Kimia') // Menambahkan filter untuk program_studi
            ->groupBy('melanjutkan_studi')
            ->pluck('total', 'melanjutkan_studi')->all();
